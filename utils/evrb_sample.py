...
def prepare_inputs_for_generation(
    ...
    **kwargs,
    ...
    if (
        ...
        position_ids = torch.arange(attention_mask.shape[-1]).unsqueeze(0).to(attention_mask)
        #-----------------------------------------------hnx-ed------------------------------------------------
...
class AnomalyDetector:
    def __init__(self, vv_threshold = None):
        self.sequence = torch.tensor([])     # 原始序列
...
def _sample(
    ...
    **model_kwargs,
    ...
    if do_ct:
        ...
        # import pdb; pdb.set_trace()
        model_kwargs['attention_mask'][:,redundant_img_loc] = 0
        ...
        for idx in range(len(model_kwargs['past_key_values'].key_cache)):
            model_kwargs['past_key_values'].key_cache[idx] = model_kwargs['past_key_values'].key_cache[idx][:,:,:img_ed]
        for idx in range(len(model_kwargs['past_key_values'].value_cache)):
            model_kwargs['past_key_values'].value_cache[idx] = model_kwargs['past_key_values'].value_cache[idx][:,:,:img_ed]
        ...
        ct_model_kwargs['attention_mask'][:,redundant_img_loc] = 1
        ct_model_kwargs['attention_mask'][:,clear_img_loc] = 0
    ...
    while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
        ...
        if is_prefill:
            if do_eos and "logits_to_keep" in model_inputs:
                model_inputs['logits_to_keep'] = int(model_inputs['input_ids'].shape[-1])
            if "output_attentions" in model_inputs:
