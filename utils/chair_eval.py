...
class CHAIR(object):
    ...
    def caption_to_words(self, caption):
        ...
        return words, node_words, idxs, double_words # words: 出现的mscoco gt 以及同义词， node words: gt; idx: gt idx;  double_words: raw gt + 同义词
...
def my_storage(cap_dict, save_path, save_name):
    result = {}
    for sentence in cap_dict['sentences']:
        result[sentence['image_id']] = sentence
...
if __name__ == '__main__':
    ...
    parser.add_argument("--coco_path", type=str, default='/mnt/nanjing3cephfs/wx-mm-spr-xxxx/neilnxhu/data/coco/annotations',
                        help="only use for regenerating CHAIR evaluator object, will be ignored if uses cached evaluator.")
    
    parser.add_argument("--save_path", type=str, default="log/results/chair_bayes.txt",
    ...
    # save_name = args.cap_file.split('/')[-1]
    # result = my_storage(cap_dict, args.save_path,save_name)
    # import pdb; pdb.set_trace()
